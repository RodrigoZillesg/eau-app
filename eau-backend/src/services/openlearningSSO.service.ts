...
class OpenLearningSSOService {
   ...
   */
  async generateSSOLink(params: SSOParams) {
    try {
   ...
   */
  async getUserCourseCompletions(openlearningUserId: string) {
    try {
   ...
   */
  async importCertificatesAsCPD(memberId: string, openlearningUserId: string) {
    try {
      ...
      for (const completion of completions) {
        ...
        const { data: existingCPD } = await supabaseAdmin
          .from('cpd_activities')
          .select('id')
        ...
        if (!existingCPD) {
          ...
          const { error: cpdError } = await supabaseAdmin
            .from('cpd_activities')
            .insert({
              member_id: memberId,
  ...
  async syncAllUsers() {
    try {
      ...
      const result = await response.json() as any;
      const openlearningUsers: OpenLearningUser[] = result.users || [];
...
export const openLearningSSOService = new OpenLearningSSOService();