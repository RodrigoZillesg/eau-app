 ...
 */
export async function requestInstitutionLink(data: LinkRequest) {
  try {
 ...
 */
export async function approveLinkRequest({ requestId, reviewedBy, notes }: ApproveData) {
  try {
    ...
    let previousInstitutionId: string | null = null;
    let previousInstitutionName: string | null = null;
    ...
    if (request.member.institution_id && request.member.institution_id !== request.institution_id) {
      ...
      const { data: prevInst } = await supabaseAdmin
        .from('institutions')
        .select('name')
      ...
      try {
        const { data: prevInstAdmins } = await supabaseAdmin
          .from('members')
          .select('email, first_name, last_name')
 ...
 */
export async function rejectLinkRequest({ requestId, reviewedBy, notes }: RejectData) {
  try {
    // 1. Fetch the request with member details
    const { data: request, error: requestError } = await supabaseAdmin
      .from('institution_link_requests')
 ...
 */
export async function getPendingLinkRequests(institutionId: string) {
  try {
 ...
 */
export async function getAllLinkRequests(institutionId: string) {
  try {
    const { data, error } = await supabaseAdmin
      .from('institution_link_requests')
      .select(`
 ...
 */
export async function unlinkFromInstitution(memberId: string) {
  try {
 ...
 */
export async function getMemberLinkStatus(memberId: string) {
  try {
    // Get member data
    const { data: member, error: memberError } = await supabaseAdmin
      .from('members')
