...
class EmailService {
     ...
     */
    static async getEnabledSMTPSettings() {
        try {
            const { data, error } = await database_1.supabaseAdmin
                .from('smtp_settings')
                .select('*')
    ...
    static async sendEmail(options) {
        try {
            ...
            const messageId = await emailLogger_service_1.EmailLoggerService.logEmail({
            ...
            await emailLogger_service_1.EmailLoggerService.logEmail({
                recipient_email: options.to,
     ...
     */
    static async saveSMTPSettings(settings) {
        try {
            // Check if settings already exist
            const { data: existingSettings } = await database_1.supabaseAdmin
                .from('smtp_settings')
                .select('id')
                .limit(1);
            let result;
