...
CREATE POLICY "Only admins can delete media" ON media_files
    FOR DELETE
...
CREATE POLICY "Only admins can manage folders" ON media_folders
    FOR ALL
    USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
...
CREATE POLICY "Only admins can delete usage tracking" ON media_usage
    FOR DELETE
    USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
